...
void ReflectExplosion(int type, vec3_t origin)
{
    ...
    for (m = 0; m < level.num_reflectors; m++)
    {
        ...
        if (org[0] < mirror->absmin[0])
        {
            continue;
        }
        if (org[0] > mirror->absmax[0])
        {
            continue;
        }
        if (org[1] < mirror->absmin[1])
        {
            continue;
        }
        if (org[1] > mirror->absmax[1])
        {
            continue;
        }
        if (org[2] < mirror->absmin[2])
        {
            continue;
        }
        if (org[2] > mirror->absmax[2])
        {
            continue;
        }
...
void ReflectTrail(int type, vec3_t start, vec3_t end)
{
    ...
    for (m = 0; m < level.num_reflectors; m++)
    {
        ...
        if (p1[0] < mirror->absmin[0])
        {
            continue;
        }
        if (p1[0] > mirror->absmax[0])
        {
            continue;
        }
        if (p1[1] < mirror->absmin[1])
        {
            continue;
        }
        if (p1[1] > mirror->absmax[1])
        {
            continue;
        }
        if (p1[2] < mirror->absmin[2])
        {
            continue;
        }
        if (p1[2] > mirror->absmax[2])
        {
            continue;
        }
...
void AddReflection(edict_t *ent)
{
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        for (m = 0; m < level.num_reflectors && !is_reflected; m++)
        {
            ...
            {
            case 0:
                org[2] = 2 * mirror->absmax[2] - ent->s.origin[2] - mirror->moveinfo.distance - 2;
                break;

            case 1:
                org[2] = 2 * mirror->absmin[2] - ent->s.origin[2] + mirror->moveinfo.distance + 2;
                break;

            case 2:
                org[0] = 2 * mirror->absmin[0] - ent->s.origin[0] + mirror->moveinfo.distance + 2;
                break;

            case 3:
                org[0] = 2 * mirror->absmax[0] - ent->s.origin[0] - mirror->moveinfo.distance - 2;
                break;

            case 4:
                org[1] = 2 * mirror->absmin[1] - ent->s.origin[1] + mirror->moveinfo.distance + 2;
                break;

            case 5:
                org[1] = 2 * mirror->absmax[1] - ent->s.origin[1] - mirror->moveinfo.distance - 2;
                break;
            }
            if (org[0] < mirror->absmin[0])
            {
