...
edict_t *CrateOnTop(edict_t *from, edict_t *ent)
{
    ...
    for ( ; from < &g_edicts[globals.num_edicts]; from++)
    {
        ...
        if (from->absmin[0] >= ent->absmax[0])
        {
            continue;
        }
        if (from->absmax[0] <= ent->absmin[0])
        {
            continue;
        }
        if (from->absmin[1] >= ent->absmax[1])
        {
            continue;
        }
        if (from->absmax[1] <= ent->absmin[1])
        {
            continue;
        }
        maxdist = VectorLength(ent->velocity) * FRAMETIME + 2.0;
        if (fabs(from->absmin[2] - ent->absmax[2]) > maxdist)
        {
            continue;
        }
        return from;
...
void crane_control_action(edict_t *control, edict_t *activator, vec3_t point)
{
    ...
    if (cargo)
    {
        cargo->gravity = 0.0;              // reset after making it float up,
    }
    // otherwise things get jammed up
    control->activator = activator;

    // if any part of crane is currently moving, do nothing.
    if (VectorLength(control->velocity) > 0.)
    {
        return;
    }
    if (VectorLength(beam->velocity) > 0.)
    {
        return;
    }
    if (VectorLength(hoist->velocity) > 0.)
    {
        return;
    }
    if (VectorLength(hook->velocity) > 0.)
    {
        return;
    }
