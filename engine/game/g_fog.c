...
void Fog(edict_t *ent)  //vec3_t viewpoint)
{
    ...
    if (level.trigger_fogs)
    {
        ...
        for (i = 1; i < level.fogs; i++)
        {
            ...
            if (viewpoint[0] < gfogs[i].ent->absmin[0])
            {
                continue;
            }
            if (viewpoint[0] > gfogs[i].ent->absmax[0])
            {
                continue;
            }
            if (viewpoint[1] < gfogs[i].ent->absmin[1])
            {
                continue;
            }
            if (viewpoint[1] > gfogs[i].ent->absmax[1])
            {
                continue;
            }
            if (viewpoint[2] < gfogs[i].ent->absmin[2])
            {
                continue;
            }
            if (viewpoint[2] > gfogs[i].ent->absmax[2])
            {
                continue;
            }
            trigger = i;
