...
void trigger_speaker_think(edict_t *self)
{
    ...
    for (i = 1; i <= maxclients->value && !touching; i++)
    {
        ...
        if (player->s.origin[0] < self->s.origin[0] + self->bleft[0])
        {
            continue;
        }
        if (player->s.origin[1] < self->s.origin[1] + self->bleft[1])
        {
            continue;
        }
        if (player->s.origin[2] < self->s.origin[2] + self->bleft[2])
        {
            continue;
        }
        if (player->s.origin[0] > self->s.origin[0] + self->tright[0])
        {
            continue;
        }
        if (player->s.origin[1] > self->s.origin[1] + self->tright[1])
        {
            continue;
        }
        if (player->s.origin[2] > self->s.origin[2] + self->tright[2])
        {
            continue;
        }
        touching = player;
...
int trigger_transition_ents(edict_t *changelevel, edict_t *self)
{
    ...
    for (i = game.maxclients + 1; i < globals.num_edicts; i++)
    {
        ...
        if (ent->s.origin[0] > self->maxs[0])
        {
            continue;
        }
        if (ent->s.origin[1] > self->maxs[1])
        {
            continue;
        }
        if (ent->s.origin[2] > self->maxs[2])
        {
            continue;
        }
        if (ent->s.origin[0] < self->mins[0])
        {
            continue;
        }
        if (ent->s.origin[1] < self->mins[1])
        {
            continue;
        }
        if (ent->s.origin[2] < self->mins[2])
        {
            continue;
        }
