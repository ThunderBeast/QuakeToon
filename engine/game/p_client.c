...
// MUD - get mud level
    if (level.mud_puddles)
    {
        ...
        for (i = game.maxclients + 1; i < globals.num_edicts && !ent->in_mud; i++)
        {
            ...
            if (!(mud->svflags & SVF_MUD))
            {
                continue;
            }
            if (ent->absmin[0] > mud->absmax[0])
            {
                continue;
            }
            if (ent->absmin[1] > mud->absmax[1])
            {
                continue;
            }
            if (ent->absmin[2] > mud->absmax[2])
            {
                continue;
            }
            if (ent->absmax[0] < mud->absmin[0])
            {
                continue;
            }
            if (ent->absmax[1] < mud->absmin[1])
            {
                continue;
            }
            if (ent->absmax[2] < mud->absmin[2])
            {
                continue;
            }
            ent->in_mud = 1;
