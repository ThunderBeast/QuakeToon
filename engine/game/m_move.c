...
qboolean SV_movestep(edict_t *ent, vec3_t move, qboolean relink)
{
    ...
    if (dist > 0.)
    {
        ...
        for (i = game.maxclients + 1; i < globals.num_edicts; i++)
        {
            ...
            if (monster_maxs[0] < laser_mins[0])
            {
                continue;
            }
            if (monster_maxs[1] < laser_mins[1])
            {
                continue;
            }
            if (monster_maxs[2] < laser_mins[2])
            {
                continue;
            }
            if (monster_mins[0] > laser_maxs[0])
            {
                continue;
            }
            if (monster_mins[1] > laser_maxs[1])
            {
                continue;
            }
            if (monster_mins[2] > laser_maxs[2])
            {
                continue;
            }
