...
void R_SetGL2D(void)
{
    ...
    if (r_speeds->value && !(r_newrefdef.rdflags & RDF_NOWORLDMODEL))     // don't do this for options menu
    {
        ...
        for (i = 0; i < lines; i++)
        {
            ...
            {
            case 0:
                n = sprintf(S, S_COLOR_ALT S_COLOR_SHADOW "%5i wcall", c_brush_calls);
                break;

            case 1:
                n = sprintf(S, S_COLOR_ALT S_COLOR_SHADOW "%5i wsurf", c_brush_surfs);
                break;

            case 2:
                n = sprintf(S, S_COLOR_ALT S_COLOR_SHADOW "%5i wpoly", c_brush_polys);
                break;

            case 3:
                n = sprintf(S, S_COLOR_ALT S_COLOR_SHADOW "%5i epoly", c_alias_polys);
                break;

            case 4:
                n = sprintf(S, S_COLOR_ALT S_COLOR_SHADOW "%5i ppoly", c_part_polys);
                break;

            case 5:
                n = sprintf(S, S_COLOR_ALT S_COLOR_SHADOW "%5i tex  ", c_visible_textures);
                break;

            case 6:
                n = sprintf(S, S_COLOR_ALT S_COLOR_SHADOW "%5i lmaps", c_visible_lightmaps);
                break;
...
//	VID_Printf (PRINT_DEVELOPER, "GL_EXTENSIONS: %s\n", glConfig.extensions_string );
    ...
    else if (strstr(vendor_buffer, "matrox"))
    {
        glConfig.rendType = GLREND_MATROX;
    }
    else if (strstr(vendor_buffer, "intel"))
    {
        glConfig.rendType = GLREND_INTEL;
    }
    else if (strstr(vendor_buffer, "sgi"))
    {
        glConfig.rendType = GLREND_SGI;
    }
    else if (strstr(renderer_buffer, "permedia"))
    {
        glConfig.rendType = GLREND_PERMEDIA2;
    }
    else if (strstr(renderer_buffer, "glint"))
    {
        glConfig.rendType = GLREND_GLINT_MX;
    }
    else if (strstr(renderer_buffer, "glzicd"))
    {
        glConfig.rendType = GLREND_REALIZM;
    }
    else if (strstr(renderer_buffer, "pcx1"))
    {
        glConfig.rendType = GLREND_PCX1;
    }
    else if (strstr(renderer_buffer, "pcx2"))
    {
        glConfig.rendType = GLREND_PCX2;
    }
    else if (strstr(renderer_buffer, "pmx"))
    {
        glConfig.rendType = GLREND_PMX;
    }
    else if (strstr(renderer_buffer, "verite"))
    {
        glConfig.rendType = GLREND_RENDITION;
    }
    else if (strstr(vendor_buffer, "sis"))
    {
        glConfig.rendType = GLREND_SIS;
    }
    else if (strstr(renderer_buffer, "voodoo"))
    {
        glConfig.rendType = GLREND_VOODOO;
    }
    else if (strstr(renderer_buffer, "gdi generic"))
    {
    ...
    if (strstr(glConfig.extensions_string, "GL_ARB_vertex_buffer_object"))
    {
         ...
         *
         *      glBindBufferARB = (void *) qwglGetProcAddress( "glBindBufferARB" );
         *      glDeleteBuffersARB = (void *) qwglGetProcAddress( "glDeleteBuffersARB" );
         *      glGenBuffersARB = (void *) qwglGetProcAddress( "glGenBuffersARB" );
         *      glBufferDataARB = (void *) qwglGetProcAddress( "glBufferDataARB" );
         *      glBufferSubDataARB = (void *) qwglGetProcAddress( "glBufferSubDataARB" );
         *      glMapBufferARB = (void *) qwglGetProcAddress( "glMapBufferARB" );
         *      glUnmapBufferARB = (void *) qwglGetProcAddress( "glUnmapBufferARB" );
         * }
         * else*/
    ...
    if (strstr(glConfig.extensions_string, "GL_ARB_fragment_program"))
    {
        ...
        {
            if (!glProgramStringARB || !glBindProgramARB ||
