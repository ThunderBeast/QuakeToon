...
float	subdivide_size = 1024; // was 240
...
#define	HASH_SIZE	MAX_POINTS_HASH
#else
#define	HASH_SIZE	64
#endif
...
unsigned HashVec (vec3_t vec)
{
	...
	x = (max_bounds + (int)(vec[0]+0.5)) >> 7; // was 4096
	y = (max_bounds + (int)(vec[1]+0.5)) >> 7; // was 4096
...
int	GetVertexnum (vec3_t v)
{
	...
	for (i=0 ; i<3 ; i++)
	{
		if ( fabs(v[i] - (int)(v[i]+0.5)) < INTEGRAL_EPSILON )
			v[i] = (int)(v[i]+0.5);
		if (v[i] < -max_bounds || v[i] > max_bounds) // was -4096, 4096
			Error ("GetVertexnum: outside bounds of +/- %i", max_bounds); // was 4096
	}
...
#if 0
{
	...
	x1 = (max_bounds + (int)(v1[0]+0.5)) >> 7; // was 4096
	y1 = (max_bounds + (int)(v1[1]+0.5)) >> 7; // was 4096
	x2 = (max_bounds + (int)(v2[0]+0.5)) >> 7; // was 4096
	y2 = (max_bounds + (int)(v2[1]+0.5)) >> 7; // was 4096
...
face_t *FaceFromPortal (portal_t *p, int pside)
{
	...
	// Knightmare- caulk/nodraw faces
	if ( !strcmp(side_brushtextures[side-brushsides].name, "system/caulk")
		|| !strncmp(side_brushtextures[side-brushsides].name, "system/nodraw", 13)
		|| !strcmp(side_brushtextures[side-brushsides].name, "common/caulk")
		|| !strncmp(side_brushtextures[side-brushsides].name, "common/nodraw", 13) )
		return NULL;
